...
class DownloadTask extends FileTask {
   ...
   * @type {boolean}
   */
  get includeOriginalStrings() { return typeof this._params.includeOriginalStrings == 'boolean' ? this._params.includeOriginalStrings : false; }
  ...
  run() {
    ...
    return Promise.all(this.locales.map(locale => {
      fileApi.get(this.fileUri, this.filePattern.replace('{{locale}}', locale), Object.assign({
